...
// ═══════════════════════════════════════════════════════════════════════════
//  DJ30 ICT — 1 YEAR BACKTEST
//  DJ30 strategy: Judas Swing — 5m pre-NY range sweep (14:00–16:00 UTC)
//    • Pre-NY range: 5m H/L from 07:00–13:55 UTC
//    • Sweep: wick beyond pre-NY H/L + close back (14:00–16:00 UTC)
...
async function run() {
  console.clear();
  console.log('\n' + chalk.bold.cyan('  ◆ DJ30 ICT — 1 YEAR BACKTEST'));
  console.log(chalk.gray('  Judas Swing  |  5m pre-NY range  |  NY KZ 14:00–16:00 UTC  |  1% risk  |  £1,000 start\n'));

  const range = oneYearRange();
  console.log(chalk.gray(`  Period: ${range.label}\n`));
  console.log(chalk.gray('  Fetching data in monthly chunks (13 months)...\n'));

  const all5m  = await fetchChunked('5min',  4500, 13); await wait(15000);
  const allH1  = await fetchChunked('1h',    750,  13); await wait(15000);
  ...
  const sep = '═'.repeat(72);
  console.log('\n' + sep);
  console.log(chalk.bold.cyan('  SIGNAL REPORT — DJ30 — 1 YEAR'));
  ...
  console.log('\n\n' + sep);
  console.log(chalk.bold.cyan('  1-YEAR SUMMARY'));
  console.log(sep);
  ...
  const reportPath = path.join(__dirname, '..', 'backtest_report_1yr_dj30.json');
  fs.writeFileSync(reportPath, JSON.stringify({
    period: range.label, generatedAt: new Date().toISOString(),
    settings: { symbol: 'DJ30/DIA', entryMethod: 'fvg_midpoint_limit', startBalance: ACCOUNT_START, riskPct: RISK_PCT*100, minConfluence: MIN_SCORE },
    account: {
  ...
  console.log(chalk.gray('\n  Report saved → backtest_report_1yr_dj30.json'));
  console.log('\n' + sep + '\n');
}

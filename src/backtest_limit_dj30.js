...
// ═══════════════════════════════════════════════════════════════════════════
//  DJ30 ICT — LIMIT ORDER BACKTEST
//  Tests whether placing a limit at the FVG midpoint after signal fires
//  produces a better outcome than market execution (next bar open).
//
//  Methodology:
//    1. Signal detected at bar i (same ICT logic)
//    2. Limit order placed at FVG midpoint (optimal entry)
//    3. Scan next FILL_WINDOW bars (12 = 1 hour) for fill
//    4. If filled: simulate trade from fill bar onwards
//    5. If not filled within window: mark as MISSED
//
//  Compared to:
//    A) Market execution (next bar open)  — already run
//    B) FVG midpoint assumed fill         — original optimistic result
// ═══════════════════════════════════════════════════════════════════════════
...
function detectFVG(candles5m, sweepDir) {
  ...
  let confirmed = false;
  if (sweepDir === 'bear') {
    confirmed = prev.high >= best.bottom && prev.close <= best.top &&
                (prev.high - best.bottom) >= best.size * 0.5;
  } else {
    confirmed = prev.low <= best.top && prev.close >= best.bottom &&
                (best.top - prev.low) >= best.size * 0.5;
  }
...
function liquidityTPs(dir, entry, risk, candles5m, h1Candles) {
  ...
  for (let i = 2; i < c5.length - 1; i++) {
    ...
    if (isLong) {
      ...
      if (eq) candidates.push({ price: Math.max(c.high, eq.high) });
    } else {
      const eq = prev.find(p => Math.abs(p.low - c.low) / c.low < 0.001);
      if (eq) candidates.push({ price: Math.min(c.low, eq.low) });
    }
  }
  ...
  const tp2 = valid[0]?.price ?? (isLong ? entry + risk*2.5 : entry - risk*2.5);
  const tp3 = valid[1]?.price ?? (isLong ? entry + risk*3.5 : entry - risk*3.5);
  return { tp2, tp3 };
}
...
function simulateOutcome(dir, entry, sl, tp1, tp2, tp3, futureCandles) {
  ...
  for (const c of futureCandles) {
    ...
    if (!tp1Hit) {
      ...
      if (tp3Hit)  return { result: 'WIN_TP3',    pnlR: 0.5*TP1_R + 0.25*2.5 + 0.25*3.5 };
...
async function run() {
  console.clear();
  console.log('\n' + chalk.bold.cyan('  ◆ DJ30 ICT — 3 MONTH BACKTEST  [LIMIT ORDER @ FVG MIDPOINT]'));
  console.log(chalk.gray(`  Fill window: ${FILL_WINDOW} bars (${FILL_WINDOW*5}min)  |  KZ only  |  80% min  |  1% risk  |  £1,000 start\n`));

  const range = threeMonthRange();
  console.log(chalk.gray(`  Period: ${range.label}\n`));
  console.log(chalk.gray('  Loading data (using cached where available)...\n'));
  ...
  for (let i = 50; i < period5m.length - 1; i++) {
    ...
    // Limit order: placed at FVG midpoint
    const limitEntry = isLong ? fvg.bottom + fvg.size * 0.5 : fvg.top - fvg.size * 0.5;
    const sl         = isLong
      ? sweep.level - sweep.level * 0.001
      : sweep.level + sweep.level * 0.001;
    const risk = Math.abs(limitEntry - sl);
    if (risk <= 0 || risk > limitEntry * 0.015) continue;

    const tp1 = isLong ? limitEntry + risk * TP1_R : limitEntry - risk * TP1_R;
    const { tp2, tp3 } = liquidityTPs(dir, limitEntry, risk, slice5m, sliceH1);

    // Scan fill window: does price return to FVG midpoint within FILL_WINDOW bars?
    ...
    if (fillIdx === -1) {
      // Limit never filled — price ran without us
      missedCount++;
      signals.push({
        ...
        fillIdx: -1, note: 'Price ran — limit never filled'
      });
      lastBar = i;
      continue;
    }
  ...
  const sep = '═'.repeat(72);
  console.log('\n' + sep);
  console.log(chalk.bold.cyan('  SIGNAL REPORT — DJ30 LIMIT ORDER — 3 MONTHS'));
  console.log(chalk.gray(`  ${range.label}  |  Fill window: ${FILL_WINDOW} bars (${FILL_WINDOW*5}min)`));
  console.log(sep);

  signals.forEach((s, idx) => {
    const isLong = s.dir === 'BUY';
    const color  = isLong ? chalk.green : chalk.red;
    const oc     = s.result === 'MISSED' ? chalk.gray
                 : s.result?.startsWith('WIN') ? chalk.green : chalk.red;

    if (s.result === 'MISSED') {
      console.log(
        '\n  ' + chalk.bold(`#${idx+1}`) + chalk.gray(` ${fmtDT(s.time)}`) +
        '  ' + color(`${isLong?'▲':'▼'} ${s.dir}`) +
        chalk.gray(`  Limit $${s.limitEntry}  ${s.score}%`) +
        '  ' + chalk.gray('MISSED — price ran without filling limit')
      );
      return;
    }

    const pnlStr = s.pnlR != null
      ? (s.pnlR > 0 ? chalk.green(`+${s.pnlR.toFixed(2)}R  +£${s.pnlGBP}`)
                    : chalk.red(`${s.pnlR.toFixed(2)}R  £${s.pnlGBP}`))
      : chalk.yellow('open');

    console.log(
      '\n  ' + chalk.bold(`#${idx+1}`) + chalk.gray(` ${fmtDT(s.time)}`) +
      '  ' + color(`${isLong?'▲':'▼'} ${s.dir}`) +
      chalk.gray(`  ${s.score}%  filled in ${s.fillBarsAfter} bar(s)`)
    );
    console.log(
  ...
  const avgFillBars = filled.length
    ? (filled.reduce((s,x)=>s+(x.fillBarsAfter||0),0)/filled.length).toFixed(1)
    : '—';

  console.log('\n\n' + sep);
  console.log(chalk.bold.cyan('  3-MONTH SUMMARY — LIMIT ORDER'));
  console.log(sep);
  console.log(chalk.gray('  Total signals:     ') + chalk.white(signals.length));
  console.log(chalk.gray('  Limit filled:      ') + chalk.green(`${filled.length}  (${fillRate}% fill rate)`));
  console.log(chalk.gray('  Limit MISSED:      ') + chalk.yellow(`${missed.length}  — price ran without us`));
  console.log(chalk.gray('  Avg fill time:     ') + chalk.white(`${avgFillBars} bars (${(parseFloat(avgFillBars)*5).toFixed(0)}min avg)`));
  ...
  console.log(chalk.gray('  Market exec (next bar open): ') + chalk.red('35% WR  -2.0% return  PF 0.92'));
  console.log(chalk.gray('  Limit @ FVG midpoint (this): ') +
    (parseFloat(wr)>=50?chalk.green:chalk.red)(`${wr}% WR  ${((balance-ACCOUNT_START)/ACCOUNT_START*100).toFixed(1)}% return  PF ${pf}  (${fillRate}% fill rate)`));

  console.log(chalk.gray('\n  Miss breakdown — what happened to the ${missed.length} missed trades:'));
  if (missed.length === 0) {
    console.log(chalk.gray('    All limits filled!'));
  } else {
    console.log(chalk.gray(`    ${missed.length} signals where price ran straight to target without pulling back`));
    console.log(chalk.gray(`    If you'd entered at market on those ${missed.length}, you'd have caught the move`));
  }

  console.log('\n' + sep + '\n');

  const reportPath = path.join(__dirname, '..', 'backtest_report_limit_dj30.json');
  fs.writeFileSync(reportPath, JSON.stringify({
    period: range.label, generatedAt: new Date().toISOString(),
    settings: { entryMethod: 'limit_fvg_midpoint', fillWindowBars: FILL_WINDOW, fillWindowMins: FILL_WINDOW*5 },
    account: {
    ...
    stats: { total: signals.length, filled: filled.length, missed: missed.length,
      ...
      avgFillBars, avgFillMins: (parseFloat(avgFillBars)*5).toFixed(0) },
    signals
  }, null, 2));
  console.log(chalk.gray('  Report saved → backtest_report_limit_dj30.json\n'));
}

...
function signalMessage(sig) {
  ...
  return [
    ...
    `🕐 <code>${time} UTC</code>`,
    ``,
    `💰 <b>ENTRY</b>     <code>AT MARKET</code>  <i>(ref: $${entry})</i>`,
    `🛑 <b>STOP LOSS</b>  <code>${pts} pts</code> from entry`,
    ``,
    `🎯 <b>TP1</b>  <code>+${Math.round(Math.abs(sig.tp1 - sig.entry))} pts</code>  <i>— close 50% here, move SL to BE</i>`,
    `🎯 <b>TP2</b>  <code>+${Math.round(Math.abs(sig.tp2 - sig.entry))} pts</code>  <i>(${sig.tp2Desc || '—'})</i>`,
    `🎯 <b>TP3</b>  <code>+${Math.round(Math.abs(sig.tp3 - sig.entry))} pts</code>  <i>(${sig.tp3Desc || 'extension'})</i>`,
    ``,
